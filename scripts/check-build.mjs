...
for (const file of ['random.html', 'js/random.js', 'matlab/untitled.m', 'matlab/untitled-original.m', 'index.html', 'style.css', 'favicon.svg', '.nojekyll', 'RUNMAT-LICENSE.txt', 'THIRD-PARTY-NOTICES.txt', 'js/app.js', 'js/model.js', 'js/worker.js', 'js/charts.js', 'vendor/runmat/index.js', 'vendor/runmat/pkg-web/runmat_wasm_web.js']) await access(path.join(dist,file));
for (const file of SOURCE_FILES) assert.deepEqual(await readFile(path.join(dist,'matlab',file)),await readFile(path.join(root,'matlab',file)));
